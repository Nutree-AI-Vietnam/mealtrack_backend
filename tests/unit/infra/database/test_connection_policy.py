...
def test_direct_pool_capacity_uses_workers_and_pool_size():
    ...
    assert policy.pool_size == 5
    assert policy.max_overflow == 3
    assert policy.worker_count == 2
    assert policy.total_capacity == 16  # 2 workers * (5 base + 3 overflow)
